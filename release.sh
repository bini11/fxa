...
#   5. If current branch is train branch, pull from origin.
#   6. Otherwise checkout existing train branch or create fresh one from master.
#   7. Generate the bumped version string.
#   8. For each of the "main" packages...
#      8.1. List commits since the last tag.
#      8.2. For each commit...
#           8.2.1. Add the commit message to a summary string.
#      8.3. Write the summary string to CHANGELOG.md.
...
if [ "$CURRENT_BRANCH" = "$TRAIN_BRANCH" ]; then
  ...
  if [ "$TRAIN_BRANCH_EXISTS" = "" ]; then
    ...
    if [ "$REMOTE_BRANCH_EXISTS" = "" ]; then
      ...
      git checkout --track -b "$TRAIN_BRANCH" "$REMOTE_BRANCH"
    fi
  else
...
if [ "$PRIVATE_BRANCH_EXISTS" = "" ]; then
  git fetch "$PRIVATE_REMOTE" "$PRIVATE_BRANCH" || true

  PRIVATE_REMOTE_BRANCH_EXISTS=`git branch -r | awk '{$1=$1};1' | grep "^$PRIVATE_REMOTE_BRANCH\$"` || true
  if [ "$PRIVATE_REMOTE_BRANCH_EXISTS" = "" ]; then
    git checkout --no-track -b "$PRIVATE_BRANCH" "$PRIVATE_REMOTE/master"
  else
    git checkout --track -b "$PRIVATE_BRANCH" "$PRIVATE_REMOTE_BRANCH"
  fi
else
  git checkout "$PRIVATE_BRANCH"
  git pull "$PRIVATE_REMOTE" "$PRIVATE_BRANCH" || true
fi

# 11. Merge train branch into the private train branch.
git merge "$TRAIN_BRANCH" -m "Merge $TRAIN_BRANCH into $PRIVATE_BRANCH"
